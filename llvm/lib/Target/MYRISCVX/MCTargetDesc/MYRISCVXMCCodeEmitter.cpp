...
getExprOpValue(const MCInst &MI, const MCExpr *Expr,
  ...
  if (Kind == MCExpr::Target) {
    ...
    switch (MYRISCVXExpr->getKind()) {
      ...
      case MYRISCVXMCExpr::CEK_LO:
        if (MIFrm == MYRISCVXII::FrmI)
          FixupKind = MYRISCVX::fixup_MYRISCVX_LO12_I;
        else if (MIFrm == MYRISCVXII::FrmS)
          FixupKind = MYRISCVX::fixup_MYRISCVX_LO12_S;
        else
          llvm_unreachable("CEK_LO used with unexpected instruction format");
        break;
      case MYRISCVXMCExpr::CEK_HI:
