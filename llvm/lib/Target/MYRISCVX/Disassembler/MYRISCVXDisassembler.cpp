...
// @{ MYRISCVXDisassembler_cpp_DecodeRegsRegisterClass
static DecodeStatus DecodeCPURegsRegisterClass(MCInst &Inst,
                                               unsigned RegNo,
                                               uint64_t Address,
                                               const void *Decoder) {
...
// @{ MYRISCVXDisassembler_cpp_decodeSImmOperand
template <unsigned N>
...
static DecodeStatus decodeUImmOperand(MCInst &Inst, uint64_t Imm,
  ...
  assert(isUInt<N>(Imm) && "Invalid immediate");
  // Sign-extend the number in the bottom N bits of Imm
  Inst.addOperand(MCOperand::createImm(Imm & ((1 << N) - 1)));
  return MCDisassembler::Success;
}


// @{ MYRISCVXDisassembler_cpp_decodeBranch20Target
static DecodeStatus decodeBranch20Target(MCInst &Inst,
                                         uint64_t Imm,
                                         uint64_t Address,
                                         const void *Decoder) {

  Inst.addOperand(MCOperand::createImm(SignExtend64<20>(Imm << 1)));
  return MCDisassembler::Success;
}
// @} MYRISCVXDisassembler_cpp_decodeBranch20Target
